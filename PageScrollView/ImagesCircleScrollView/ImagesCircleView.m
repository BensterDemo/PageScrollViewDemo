...
- (void)setImageInfos:(NSArray *)imageInfos {
    ...
    for (int i = 0; i < _arrayShowInfos.count; i++) {
        ...
        if (nullImage) {
            ...
            if (_imagesCircleDelegate && [_imagesCircleDelegate respondsToSelector:@selector(scrollImagesFromImageView:info:)]) {
                [_imagesCircleDelegate scrollImagesFromImageView:imageView info:_arrayShowInfos[i]];
            }
